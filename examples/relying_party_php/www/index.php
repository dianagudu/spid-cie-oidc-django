...
$f3->route([
    ...
    try {
        $logger->log('OIDC', 'Resolving trustchain for ' . $ta_id . ' ' . $op_id);
        $trustchain = new TrustChain($config, $rp_database, $op_id, $ta_id);
        $logger->log('OIDC', 'Trustchain: ' . json_encode($trustchain));
...
$f3->route([
    ...
    try {
        ...
        $logger->log('OIDC', 'Token request: ' . json_encode($tokenRequest));

        $tokenResponse = $tokenRequest->send($token_endpoint, $code, $code_verifier);
        $logger->log('OIDC', 'Token response: ' . json_encode($tokenResponse));
        $access_token = $tokenResponse->access_token;

        $userinfoRequest = new UserinfoRequest($config, $configuration->metadata->openid_provider, $hooks);
        $logger->log('OIDC', 'Userinfo request: ' . json_encode($userinfoRequest));
        $userinfoResponse = $userinfoRequest->send($userinfo_endpoint, $access_token);

        $logger->log('OIDC', 'Userinfo response: ' . json_encode($userinfoResponse));
